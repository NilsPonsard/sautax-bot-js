        ...
        switch (args[0]) {
            ...
            case "stop":
                if (mcModos.lastIndexOf(msg.author.id) !== -1) {
                    ...
                    exec(`ssh pixelmon@10.188.27.48 'screen -S pixel -p 0 -X stuff "stop^M"'`, (err, stdout, stderr) => {
                        console.log(err, stdout, stderr)
                    })
                } else {
                    msg.reply("vous n'avez pas la permission")
            ...
            case "say":
                if (mcModos.lastIndexOf(msg.author.id) !== -1) {
                    ...
                    exec(`ssh pixelmon@10.188.27.48 'screen -S pixel -p 0 -X stuff "say ${args.slice(1).join(" ")} ^M"'`, (err, stdout, stderr) => {
                        console.log(err, stdout, stderr)
                    })
                } else {
                    msg.reply("vous n'avez pas la permission")
